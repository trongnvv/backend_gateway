module.exports = {
  ...
  init: function (pluginContext) {
    pluginContext.registerPolicy(require('./policies/backend-policy'));
    pluginContext.registerPolicy(require('./policies/tracer-policy'));
    pluginContext.registerCondition(require('./conditions/url-match'));
    pluginContext.registerGatewayRoute((app) => {
      app.use((req, res, next) => {
        console.log('Pass gateway backend: ', req.url);
        next();
      })
    });
